        ...
        private void guardar_datos_Precargados()
        {
            ...
            try
            {
                foreach (DataGridViewRow row in datalistado.Rows)
                {
                    ...
                    //cmd.Parameters.AddWithValue("@id_subcategoria", 1);
                    //cmd.Parameters.AddWithValue("@id_marca", 1);
                    ...
                    try
                    {
                        ...
                        producto.NombreProducto = descripcion;
                        producto.ImagenProducto = imagen;
                        producto.NombreImagen = lblIcoProd.Text;
                        ...
                        producto.FechaVencimiento = Convert.ToString(DateTime.Today);
                        producto.PrecioCompra = 0;
                        producto.PrecioVenta = precio_venta;
                        producto.Codigo = CODIGO;
                        producto.PrecioMayor = 0;
                        producto.CantidadMayor = 0;
                        producto.Impuesto = 0;
                        producto.Estado = "ACTIVADO";
                        producto.FechaRegistro = DateTime.Now;
                        producto.TipoProducto = "GRANEL";

                        inventario.Fecha = DateTime.Today;
                        inventario.Motivo = "Registro Excel de Producto";
                        ...
                        inventario.IdCaja = Convert.ToInt32(formProductos.cajastatic);


                        if (funcionProducto.InsertarExcel(producto, inventario) == true)
                        {
                            //MessageBox.Show("¡Importación exitosa!", "Bodega Martinez", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            //Dispose();
                        }
                    }
                    catch (Exception ex)
