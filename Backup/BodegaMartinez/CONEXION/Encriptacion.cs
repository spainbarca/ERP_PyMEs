...
namespace BodegaMartinez.CONEXION
{
    class AES
    {
        //Inicializando variables de encriptacion
        private byte[] Encrypt(byte[] clearData, byte[] Key, byte[] IV)
        {
            MemoryStream ms = new MemoryStream();
            Rijndael alg = Rijndael.Create();
            alg.Key = Key;  //mi llave para cifrar
            alg.IV = IV;    //indica que sera simetrico
            CryptoStream cs = new CryptoStream(ms, alg.CreateEncryptor(), CryptoStreamMode.Write); //haremos una tarea criptografica
        ...
        //La accion para encriptar

        //Data -> la frase que voy a encriptar
        //Password -> mi llave de encriptado
        //Bits -> que longitud de caracteres quiero
        public string Encrypt(string Data, string Password, int Bits)
        {
            ...
            {
                byte[] encryptedData = Encrypt(clearBytes, pdb.GetBytes(64), pdb.GetBytes(16));
                return Convert.ToBase64String(encryptedData);
            }
            else
            {
        ...
        //Inicializando o declaro variables de desencriptacion
        ...
        public string Decrypt(string Data, string Password, int Bits)
        {
            try
            {
                ...
                {
                    byte[] decryptedData = Decrypt(cipherBytes, pdb.GetBytes(64), pdb.GetBytes(16));
                    return System.Text.Encoding.Unicode.GetString(decryptedData);
                }
                else
                {
